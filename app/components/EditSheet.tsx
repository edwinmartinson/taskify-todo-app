...
import { deleteTodo, updateTodo } from "~/state/redux/todoSlice";
import { logError } from "~/lib/utils";
import { useTodo, useTodoActions } from "~/state/zustand/store";
...
export function ReduxEditSheet({ taskId }: EditSheetProps) {
  ...
  const task = useSelector((state: RootState) => {
    return state.todos.value.filter((t) => t.id === taskId)[0];
  });
...
export function ZustandEditSheet({ taskId }: EditSheetProps) {
  const titleRef = useRef<HTMLInputElement | null>(null);
  const descriptionRef = useRef<HTMLTextAreaElement | null>(null);
  const isCompletedRef = useRef<HTMLButtonElement | null>(null);
  const closeButtonRef = useRef<HTMLButtonElement | null>(null);

  const task = useTodo(taskId);
  const { delete: remove, update } = useTodoActions();
...
export function JotaiEditSheet({ taskId }: EditSheetProps) {
  const titleRef = useRef<HTMLInputElement | null>(null);
  const descriptionRef = useRef<HTMLTextAreaElement | null>(null);
  const isCompletedRef = useRef<HTMLButtonElement | null>(null);
  const closeButtonRef = useRef<HTMLButtonElement | null>(null);
